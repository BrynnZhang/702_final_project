...
ggplot(rq1_panel3, aes(x = Gini, y = LifeExp, color = Region)) +
  ...
  labs(
    ...
    y = "Life expectancy at birth (years)",
    color = "Region",
    title = "Figure 2. Life expectancy vs. income inequality (Gini index) by region"
  ) +
  theme_minimal() +
  theme(legend.position = "bottom")
...
We estimate an ordinal logistic regression with the CPIA environmental sustainability rating as the ordered outcome and the Gini index, log GDP per capita, and year fixed effects as predictors, which accounts for the ordered nature of the score and absorbs common shocks or changes in rating practices over time. The results (Table 2) show that both inequality and income are positively associated with higher sustainability ratings. The Gini coefficient is about 0.074, implying an odds ratio of roughly 1.08 per one-point increase and approximately a doubling of the odds of a higher rating for a 10-point increase in inequality. The log GDP coefficient is about 0.84, corresponding to an odds ratio of about 2.3, so a one-unit increase in log GDP per capita (roughly a 2.7-fold increase in income)is associated with more than double the odds of being in a higher sustainability category. Both associations are statistically significant.
...
Inequality warrants focused attention in contexts such as Sub-Saharan Africa, where it is closely linked to poorer health outcomes, but it should not be used in isolation as a proxy for environmental sustainability or average health performance in other regions without supporting country-level evidence.
...
2.  An environmental sustainability panel for Research Question 2, linking inequality, economic development, and CPIA environmental sustainability ratings.

All data are drawn from publicly available international sources:

-   Income inequality (Gini index): UN/World Bank inequality series, downloaded from UNdata (https://data.un.org).

-   GDP per capita (PPP, constant international dollars): World Bank World Development Indicators series NY.GDP.PCAP.PP.CD, accessed via UNdata (https://data.un.org/Data.aspx?d=WDI&f=Indicator_Code:NY.GDP.PCAP.PP.CD).

-   Life expectancy at birth (years): UN life table data from UNdata, using the “Medium” variant.

-   Environmental sustainability ratings: World Bank Country Policy and Institutional Assessment (CPIA), sustainability criterion, pre-processed into a country–year dataset.
...
For the sustainability panel, we construct a sustainability panel where each observation has a CPIA environmental sustainability rating, Gini, and GDP per capita for the same country and year. CPIA scores are only reported for IDA-eligible low and lower-middle-income countries, so the panel is restricted to that group from the outset.
...
-   The coefficient on Gini is small and negative in this specification. Once we control for income level and year, higher inequality is weakly associated with lower life expectancy.
...
-   The main Gini and log_GDP coefficients correspond to the reference region (for example, East Asia & Pacific).
...
Refitting the interaction model without these high-leverage points produced similar coefficients for Gini and log(GDP), so we retained the full-sample interaction model as the primary specification.

# **Appendix C. Models for Research Question 2**

## **C.1 Modelling approach and variables**

The outcome for Research Question 2 is the CPIA environmental sustainability rating, which is:
...
-   The Gini odds ratio \> 1 implies that, within this sample of low and lower-middle-income countries, higher income inequality is associated with higher odds of being in a higher CPIA sustainability category.
...
-   The relationship with Gini is weaker but still positive.
...
-   The Gini series is reasonably broad, but there are still missing country–years, especially for low-income countries and earlier periods. This leads to dropping some observations when constructing the panels.

-   **Next step:** supplement the current Gini series with other harmonized sources (for example SWIID or LIS) where available and test whether the main coefficients change when the panel becomes denser. This would mainly improve precision and robustness rather than fundamentally changing the modelling strategy.

**2. Additional covariates as sensitivity checks**

-   The current models focus on the main story (inequality, income, and outcomes) plus time and region controls. Other plausible confounders (education, health spending, governance) are not included, mainly to keep the models interpretable.
...
**2. Exploring mild non-linearity in Gini and log(GDP)**
...
-   **Next step:** add simple spline terms or quadratic terms for log(GDP) and/or Gini in secondary models. If these terms are small, we can report that the linear approximation is adequate; if they matter, we can describe the relationship as slightly non-linear while still keeping the interpretation straightforward.
